...
export default function PerfilPage() {
      ...
      try {
        ...
        if (authResponse.ok) {
          ...
          if (partidosResponse.ok) {
            ...
            const partidosNoProcesados = partidosData.filter((partido: Partido) => !partido.procesado)
            ...
            if (partidosNoProcesados.length > 0) {
              ...
              let updatedCategoria = Number.parseInt(user.nivel || "8") // Categoría inicial (8 es la peor)
  ...
  useEffect(() => {
    const calcularEficaciaCompañeros = () => {
      ...
      const eficacia = Object.keys(compañerosStats).reduce(
        (acc, compañero) => {
  ...
  const handleAddPartido = async () => {
    ...
    const resultado = scores
      .slice(0, Number.parseInt(numSets))
      .map((set) => set.join("-"))
      .join(" - ")
    const partidoData = {
      userId: userData?.id,
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 flex flex-col items-center p-4 bg-gradient-to-b from-green-50 to-white">
        <div className="w-full max-w-4xl mb-6">
          <h1 className="text-3xl font-bold text-green-800 mb-6">Bienvenido, {userData.firstName}</h1>
          ...
          <Tabs defaultValue="perfil" className="w-full" onValueChange={setActiveTab}>
                  ...
                  <div className="space-y-4">
                    {userData.nivel && (
                      <div className="flex items-center">
                        <User className="w-5 h-5 mr-2 text-gray-500" />
                        <p>
                          <strong>Categoría:</strong> {userData.nivel}
                        </p>
                      </div>
                    )}
                    <div className="space-y-2">
                      <Label htmlFor="progress" className="block text-green-800 font-bold">
                        Progreso en la Categoría
                      </Label>
                    ...
                    {userData.preferredSide && (
                      <div className="flex items-center">
                        <User className="w-5 h-5 mr-2 text-gray-500" />
                        <p>
                          <strong>Lado preferido:</strong> {userData.preferredSide}
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <TabsContent value="amigos" className="space-y-4">
              <Card className="shadow-lg border-green-100">
                <CardHeader className="bg-green-50 border-b border-green-100">
                  <CardTitle className="text-2xl font-bold text-green-800">Mis Amigos</CardTitle>
                </CardHeader>
                <CardContent className="pt-6 space-y-4">
                  ...
                  {/* Lista de Amigos */}
                  <div className="mt-6">
                    <h3 className="text-lg font-semibold mb-3 text-green-700">Amigos Actuales</h3>
                    {friends.length > 0 ? (
                      <ul className="divide-y divide-gray-200">
                        {friends.map((friend) => (
                          <li key={friend.id} className="py-3 flex justify-between items-center text-gray-800">
                            <span>
                              <strong>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg text-center">
                        ...
                        <p className="text-sm text-gray-400 mt-2">
                          ¡Explora nuevos perfiles para conectar con otros jugadores!
                        </p>
                      </div>
                    )}
                  </div>
            ...
            <TabsContent value="estadisticas" className="space-y-4">
              <Card className="shadow-lg border-green-100">
                <CardHeader className="bg-green-50 border-b border-green-100">
                  <CardTitle className="text-2xl font-bold text-green-800">Estadísticas de Partidos</CardTitle>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-green-50 p-4 rounded-lg">
                    <div className="text-center p-3 bg-white rounded-lg shadow-sm">
                      <p className="text-sm text-gray-500">Partidos Jugados</p>
                      <p className="text-3xl font-bold text-green-700">{partidos.length}</p>
                    </div>
                    <div className="text-center p-3 bg-white rounded-lg shadow-sm">
                      <p className="text-sm text-gray-500">Partidos Ganados</p>
                      <p className="text-3xl font-bold text-green-700">{partidos.filter((p) => p.ganado).length}</p>
                    </div>
                    <div className="text-center p-3 bg-white rounded-lg shadow-sm">
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="mb-8">
                      <h3 className="font-bold text-green-800 text-lg mb-3">Eficiencia Total</h3>
                      <p className="text-gray-600 text-sm mb-4">
                        Este gráfico muestra el porcentaje de partidos ganados y perdidos en relación al total de
                        partidos jugados.
                      </p>
                    ...
                    <div className="mb-8">
                      <h3 className="font-bold text-green-800 text-lg mb-3">Eficiencia con Compañeros</h3>
                      <p className="text-gray-600 text-sm mb-4">
                        Este gráfico muestra el porcentaje de victorias alcanzado con los 5 compañeros más frecuentes.
                      </p>
                      <div className="flex justify-center">
                    ...
                    {/* Gráfico de líneas */}
                    <div className="mb-8">
                      <h3 className="font-bold text-green-800 text-lg mb-3">Historial de Victorias Acumuladas</h3>
                      <p className="text-gray-600 text-sm mb-4">
                        Este gráfico muestra cómo evolucionaron tus partidos jugados/ganados a lo largo del tiempo.
                      </p>
                    ...
                    <div className="mb-8">
                      <h3 className="font-bold text-green-800 text-lg mb-3">Rachas de Partidos</h3>
                      <p className="text-gray-600 text-sm mb-4">
                        Aquí puedes ver tus rachas más largas de partidos ganados y perdidos.
                      </p>
                  ...
                  <CardTitle className="text-2xl font-bold text-green-800 flex items-center justify-between">
                    ...
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                      <DialogTrigger asChild>
                        <Button variant="outline" size="icon" className="bg-green-100 hover:bg-green-200">
                      ...
                      <DialogContent className="sm:max-w-[425px]">
                        ...
                        <div className="grid gap-4 py-4">
                          ...
                          <div className="grid gap-2">
                            ...
                            {scores.slice(0, Number.parseInt(numSets)).map((set, setIndex) => (
                              <div key={setIndex} className="flex flex-col gap-2">
                                ...
                                <div className="flex justify-between items-center">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button variant="outline" size="sm" onClick={() => handleScoreClick(setIndex, 0)}>
                                      {set[0]}
                                    </Button>
                                    <span className="text-sm font-medium">-</span>
                                    <Button variant="outline" size="sm" onClick={() => handleScoreClick(setIndex, 1)}>
                                      {set[1]}
                                    </Button>
                                  </div>
                                  <span className="text-xs">
                                    {jugadores[2]} / {jugadores[3]}
                                  </span>
                                </div>
                              </div>
                            ))}
                  ...
                  {partidos.length > 0 ? (
                    <div className="divide-y divide-gray-200">
                      {partidos.map((partido) => (
                        <div key={partido.id} className="py-4 grid grid-cols-1 md:grid-cols-2 gap-2">
                          <div>
                            <p className="font-medium">{new Date(partido.fecha).toLocaleDateString()}</p>
                            <p className="text-sm text-gray-600">{partido.jugadores}</p>
                          </div>
                          <div className="flex flex-col md:items-end">
                            <p className="text-sm">{partido.resultado}</p>
                            <span
                              className={`text-xs px-2 py-1 rounded-full inline-flex items-center ${partido.ganado ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                            >
                    ...
                    <div className="text-center py-8 bg-gray-50 rounded-lg">
                      ...
                      <p className="text-sm text-gray-400 mt-2">
                        Haz clic en el botón + para añadir tu primer partido.
                      </p>
                    </div>
                  )}
                </CardContent>
